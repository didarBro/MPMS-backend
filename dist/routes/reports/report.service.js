...
async function getSummary() {
    const [projectStats, taskStats, timeAgg] = await Promise.all([
        db_1.prisma.project.groupBy({ by: ["status"], _count: true }),
        db_1.prisma.task.groupBy({ by: ["status"], where: { parentId: null }, _count: true }),
        db_1.prisma.timeLog.aggregate({ _sum: { hours: true } }),
    ]);
...
async function getProjectReports() {
    ...
    return projects.map((p) => {
        ...
        const loggedHours = Math.round(tasks.flatMap((t) => t.timeLogs).reduce((s, l) => s + l.hours, 0) * 10) / 10;
...
async function getUserReports() {
    ...
    return users.map((u) => {
        ...
        const loggedHours = Math.round(u.timeLogs.reduce((s, l) => s + l.hours, 0) * 10) / 10;
...
exports.reportService = { getSummary, getProjectReports, getUserReports };
